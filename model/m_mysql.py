...
def create_rdb_conn():
    return create_db_conn(ar.rdb)

def create_wdb_conn():
    return create_db_conn(ar.wdb)
...
ar.rdb_pool_cleaner = Db_Pool_Cleaner(rdb_pool, rdb_pool_lock, 1, 1) #3600, 100)
ar.wdb_pool_cleaner = Db_Pool_Cleaner(wdb_pool, wdb_pool_lock, 1, 1) #3600, 100)
def init_db_pool():
    _init_db_pool(rdb_pool, rdb_pool_num, create_rdb_conn)
    _init_db_pool(wdb_pool, wdb_pool_num, create_wdb_conn)
    ar.rdb_pool_cleaner.start()
    ar.wdb_pool_cleaner.start()
