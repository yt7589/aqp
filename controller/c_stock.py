...
class CStock(object):
    ...
    def get_stocks():
        ...
        for j in range(rec_nums):
            ...
            print('area_id type:{0}'.format(type(area_id)))
            industry_id = CStock.process_industry(rec[4])
            stock_id = CStock.process_stock(rec[0], rec[1], rec[2], area_id, industry_id, rec[5])
            print(flds)
            if j > 5:
                break
    ...
    @staticmethod
    def process_stock(ts_code, symbol, stock_name, area_id, industry_id, list_date):
        stock_id = MStock.get_stock_id_by_name(stock_name)
        if stock_id > 0:
            print('股票存在：{0}'.format(stock_name))
        else:
            stock_id = MStock.add_stock(ts_code, symbol, stock_name, area_id, industry_id, list_date)
