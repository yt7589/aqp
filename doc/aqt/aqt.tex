...
我们首先创建一个人工数据集，如下所示：v1
\begin{equation}
\begin{aligned}
\boldsymbol{a}_i \in R^{60000} \sim \mathcal{N}(0, 1), i \in {1, 2, 3, 4, 5} \\
\boldsymbol{\delta} \in R^{60000} \sim \mathcal{N}(0, 1) \\
\hat{\boldsymbol{a}}_6 = c_1 \cdot \boldsymbol{a}_1 + c_2 \cdot \boldsymbol{a}_2 + 
c_3 \cdot \boldsymbol{a}_3 + c_4 \cdot \boldsymbol{a}_4 + c_5 \cdot \boldsymbol{a}_5 \\
\boldsymbol{a}_6 = \hat{\boldsymbol{a}}_6 + \boldsymbol{\delta} \\
X = [\boldsymbol{a}_1, \boldsymbol{a}_2, \boldsymbol{a}_3, \boldsymbol{a}_4, 
    \boldsymbol{a}_5, \boldsymbol{a}_6] \\
y^{(m)}_{i} = \begin{cases}
    0, \quad \hat{a}^{(m)}_6 - a^{(m)}_6 > 0.3 \\
...
\subsubsection{生成数据集}
我们将比特币行情数据，以1000个时间点为单位，生成一个数据集。我们用第1个数据集训练XGBoost模型。
...
我们在有了数据集之后，就可以使用这个数据集来训练XgboostAgeng了。由于在这里我们不是传统的监督学习场景，所以没有准备
...
class FmeXgbAgent(object):
    ...
    def train_baby_agent(self):
        ...
        plt.show()
\end{lstlisting}
由于我们在XGBoost的demo中讲述过这部分代码，在这里就不具体讲述了。这里我们选取了1000个时间点，共训练2000个epoch，
运行结果如下所示：
\begin{figure}[H]
