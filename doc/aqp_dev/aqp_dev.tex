...
\lstset{language=PYTHON, caption={读出单支股票信息}, label={c000021}}
\begin{lstlisting}
...
\lstset{language=PYTHON, caption={数据集归一化}, label={c000022}}
\begin{lstlisting}
# app.asde.asde_ds.py
    @staticmethod
...
化的方法，如下所示：
\lstset{language=PYTHON, caption={获取股票池信息}, label={c000023}}
\begin{lstlisting}
...
\lstset{language=PYTHON, caption={训练svm模型}, label={c000024}}
\begin{lstlisting}
    # 训练初始模型
    for stock in stocks:
        stock['svm'] = AsdeSvm()
...
\lstset{language=PYTHON, caption={策略类之机器模型训练}, label={c000025}}
\begin{lstlisting}
# A股日线策略类
...
\subsubsection{账户历史信息维护}
\paragraph{将t\_account表中内容写入t\_account\_hist表}
股票总资产的统计：首先获取用户的持股列表，求出每支股票本日收盘价，计算每支股票的资产，然后求和。
\paragraph{添加c\_account中接口}

\subsubsection{绘制收益曲线}
我们可以以天为单位，绘制用户的总资产变化情况折线图。在用户总资产日变化曲线绘制中，有两个难点问题，分别是
中文显示问题和横坐日期显示问题，具体处理方法如下所示：
\lstset{language=PYTHON, caption={总资产变化曲线绘制技术}, label={c000026}}
\begin{lstlisting}
    def draw_curve_demo():
        dates = []
        idx = 0.1
...
\begin{figure}[H]
	...
	\label{f000003}
	\centering
	\includegraphics[height=3cm]{images/f000003}
\end{figure}
